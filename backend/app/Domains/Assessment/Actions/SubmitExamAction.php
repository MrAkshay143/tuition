...
class SubmitExamAction
{
    ...
    {
        return DB::transaction(function () use ($exam, $user, $answers) {
            ...
            if ($attempt->submitted_at) {
                throw new \Exception("Exam already submitted", 400);
            }

            $now = Carbon::now();
            if ($exam->duration_minutes > 0 && $attempt->started_at) {
                $maxEndTime = Carbon::parse($attempt->started_at)->addMinutes($exam->duration_minutes)->addMinutes(2);
                if ($now->greaterThan($maxEndTime)) {
                    throw new \Exception("Exam submission time has expired", 400);
                }
            }

            if ($exam->ends_at && $now->greaterThan(Carbon::parse($exam->ends_at)->addMinutes(2))) {
                throw new \Exception("Exam window has closed", 400);
            }
