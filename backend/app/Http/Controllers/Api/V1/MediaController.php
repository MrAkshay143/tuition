...
class MediaController extends \App\Http\Controllers\ApiController
{
    ...
    public function stream(Request $request, int $id, string $segment = null)
    {
        if (!$request->hasValidSignature()) {
            return $this->error('Invalid or expired stream signature.', 403);
        }
            ...
            if ($lesson && !$lesson->is_free_preview) {
                ...
                if ($course && $request->user()) {
                    if (\Illuminate\Support\Facades\Gate::denies('viewLesson', $course)) {
                        return $this->error('Unauthorized: Active course enrollment required.', 403);
                    }
                }
            }
        }
        ...
        // If segment is null, serve the m3u8 playlist or file
        if (!$segment) {
            $path = $media->path; // e.g. hls/1/playlist.m3u8
            if (empty($path) || !$providerInstance->exists($path)) {
                if (!empty($media->url)) {
                    return redirect()->away($media->url);
                }
                $fallback = config('app.fallback_video_url', '');
                return $fallback ? redirect()->away($fallback) : response()->json(['error' => 'Media not found'], 404);
            }

            // If it's not an m3u8 playlist, just return the file (e.g. raw mp4)
            if (!str_ends_with($path, '.m3u8')) {
                return $providerInstance->response($path);
            }

            $content = $providerInstance->get($path);
        ...
        if (!$providerInstance->exists($segmentPath)) {
            return $this->error('Segment not found.', 404);
        }
        
        return $providerInstance->response($segmentPath);
    }
}
