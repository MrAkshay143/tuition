...
class StudentLearningController extends ApiController
{
    use ApiResponse;
     ...
     * Returns the authenticated user's saved progress (watched_seconds) for a lesson.
     */
    public function lessonProgress(Request $request, int $id): JsonResponse
    {
        $user = $request->user();
        $progress = \App\Domains\Learning\Models\LessonProgress::where('user_id', $user->id)
            ->where('lesson_id', $id)
            ->first();
