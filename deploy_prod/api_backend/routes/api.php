...
Route::middleware(['auth:sanctum', 'active', \App\Http\Middleware\ValidateSessionBinding::class])->group(function () {
    ...
    Route::prefix('bundle')->group(function () {
        Route::get('dashboard',          [BundleController::class, 'teacherDashboard'])->middleware('role:admin,teacher');
        Route::get('student-dashboard',  [BundleController::class, 'studentDashboard'])->middleware('role:student,admin');
        Route::get('student-profile/{id}',[BundleController::class, 'studentProfile'])->middleware('role:admin,teacher,student');
        Route::get('admin-overview',     [BundleController::class, 'adminOverview'])->middleware('role:admin');
    });

    Route::get('students/{id}/devices', [StudentController::class, 'devices'])->middleware('role:admin,teacher,student');

    // ── Students (teacher manages) ───────────────────────────────────────────
    Route::prefix('students')->middleware('role:admin,teacher')->group(function () {
    ...
    // ── Batches ──────────────────────────────────────────────────────────────
    Route::prefix('batches')->middleware('role:admin,teacher')->group(function () {
    ...
    Route::middleware('role:student,admin')->group(function () {
        ...
        Route::post('student/assignments/{id}/submit', [\App\Http\Controllers\Api\V1\AssignmentController::class, 'submit'])->middleware('role:student');

        // Exams (Student)
        Route::get('student/exams', [\App\Http\Controllers\Api\V1\ExamController::class, 'studentIndex']);
        Route::post('student/exams/{id}/start', [\App\Http\Controllers\Api\V1\ExamController::class, 'start'])->middleware('role:student');
        Route::post('student/exams/{id}/submit', [\App\Http\Controllers\Api\V1\ExamController::class, 'submit'])->middleware('role:student');

        // Live Classes (Student)
        Route::get('student/live-classes', [\App\Http\Controllers\Api\V1\LiveClassController::class, 'index']);
        Route::post('live-classes/{id}/attendance', [\App\Http\Controllers\Api\V1\LiveClassController::class, 'recordAttendance'])->middleware('role:student');
    ...
    // Idempotent position/progress sync (accessible by student role)
