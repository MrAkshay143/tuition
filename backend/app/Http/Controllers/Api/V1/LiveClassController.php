...
class LiveClassController extends ApiController
{
    ...
    public function show(Request $request, $id, \App\Domains\Core\Services\AcademicAccessService $accessService)
    {
        $liveClass = LiveClass::with('batches')->findOrFail($id);

        if (!$accessService->hasAccessToLiveClass($request->user(), $liveClass)) {
    ...
    public function recordAttendance(
        ...
        $id,
        \App\Domains\Core\Services\AcademicAccessService $accessService
    ) {
        $liveClass = LiveClass::findOrFail($id);
